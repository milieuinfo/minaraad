FacetedEdit.YearWidget = function(wid){
  var self = this;
  this.wid = wid;
...
FacetedEdit.YearWidget.prototype = {
  count: function(){
...
FacetedEdit.initializeYearWidget = function(){
  jQuery('div.faceted-year-widget').each(function(){
      var wid = jQuery(this).attr('id');
      wid = wid.split('_')[0];
      FacetedEdit.Widgets[wid] = new FacetedEdit.YearWidget(wid);
  });
};
...
jQuery(document).ready(function(){
  jQuery(FacetedEdit.Events).bind(
    ...
    FacetedEdit.initializeYearWidget);
});
