/* Section Widget
*/
Faceted.SectionWidget = function(wid){
  this.wid = wid;
  ...
  if(this.maxitems){
    this.fieldset.collapsible({
      ...
      elements: 'li:not(.faceted-section-item-zerocount)'
    });
  }
};

Faceted.SectionWidget.prototype = {
  ...
  criteria_body: function(){
    ...
    label = label.text();
    var link = jQuery('<a href="#">[X]</a>');
    var span = jQuery('<span class="facted-section-criterion">');
  ...
  count_update: function(data, sortcountable){
    ...
    jQuery(lis).each(function(){
      var li = jQuery(this);
      li.removeClass('faceted-section-item-disabled');
      li.removeClass('faceted-section-item-zerocount');
      ...
      if(!value){
        li.addClass('faceted-section-item-disabled');
        if(context.widget.hasClass('faceted-zero-count-hidden')){
          li.addClass('faceted-section-item-zerocount');
        }
...
Faceted.initializeSectionWidget = function(evt){
  jQuery('div.faceted-section-widget').each(function(){
    var wid = jQuery(this).attr('id');
    wid = wid.split('_')[0];
    Faceted.Widgets[wid] = new Faceted.SectionWidget(wid);
  });
};
...
jQuery(document).ready(function(){
  jQuery(Faceted.Events).bind(
    ...
    Faceted.initializeSectionWidget);
});
